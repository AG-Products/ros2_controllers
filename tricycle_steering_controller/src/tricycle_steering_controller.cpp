...
controller_interface::CallbackReturn TricycleSteeringController::configure_odometry()
{
  ...
  const double steer_offset = tricycle_params_.steer_offset;
  const double phi_dot = tricycle_params_.phi_dot;
  const double wheel_velocity_limit = tricycle_params_.wheel_velocity_limit;

  if (params_.front_steering)
  {
    odometry_.set_wheel_params(
      rear_wheels_radius, wheelbase, steer_offset, wheel_track, wheel_velocity_limit, phi_dot);
  }
  else
  {
    odometry_.set_wheel_params(
      front_wheels_radius, wheelbase, steer_offset, wheel_track, wheel_velocity_limit, phi_dot);
  }
