...
controller_interface::CallbackReturn TricycleSteeringController::configure_odometry()
{
  ...
  {
    odometry_.set_wheel_params(rear_wheels_radius, wheelbase, steer_offset, wheel_track);
  }
  else
  {
    odometry_.set_wheel_params(front_wheels_radius, wheelbase, steer_offset, wheel_track);
  }
